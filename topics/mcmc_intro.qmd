...
Monte Carlo integration works a little bit differently. 

Let's start
by approximating the number $\pi$. We will do this by making use of the fact
that the area of a circle is $\pi r^2,$ where $r$ is the radius of a given circle.
...
Another way of saying this is that $\pi$ = $4 \times$ Area(circle)/Area(rectangle).
...
# I'll throw 100 darts:
countDartsInside(100)
```
...
# I will throw 1000 darts at the board:
calculatePi(1000)
```

# Optional coding exercise:
Can you modify the code above
to color in the darts according to whether they fall inside
or outside the circle?

## Question:
How many darts do you need to throw to approximate $\pi$
...
example), but annoyingly our functions
have different heights. We would need to customize our dartboard
...
taught a nice theorem called **The Mean Value Theorem for Integrals**. 
...
values to approximate $\bar{f}$. 

I will now randomly choose 50000 points between $a$ and $b$, evaluate our functions 
at each of these points, and take averages to get $\bar{f}$, $\bar{g}$, and $\bar{h}$:
...
you to calculate the definite integral with $a=5$, $b=\infty$.
...
We are trying to calcualte the area under the curve to the right of the red dot at x=5. That area is obviously tiny.

There are some challenges to obtaining a Monte Carlo estimate here. I probably can't use the runif function anymore
because it requires a maximum value that is finite, for one.

Now, you might be saying to yourself, "Well, R has the ability to simulate
variables from a standard normal distribution, and our h(x) is just the PDF
...
"Okay," I reply. "You go right on ahead and do that."
...
The problem is that we are trying to simulate extremely rare events to calculate this probability. This is not going to work. We need a workaround.

This is what [Importance Sampling](https://en.wikipedia.org/wiki/Importance_sampling) is designed to accommodate. 

The key is to multiply by 1 (this is a surprisingly common mathematical trick):
...
Seems trivial at first glance, but the interpretation of these two integrals is different:

In the first line, $\int h(x) dx$ means to sample x uniformly, and then average the $h(x_i)$

In the second line, $\int h(x)/w(x) dw$ means to sample x from a PDF $w(x)$, instead of uniformly,
...
getimportanceest(100)
```

We can compare this with R's built in function (i.e. the "ground truth") for calculating the tail probability:
...
# Exercise:
What happens if you change the shift in $w(x)$ to be something
different from 5?
...
Mostly we sampled from the uniform distribution, but the importance sampling example
...
The **target distribution** is the distribution we would like to sample from directly, but can't. We say that it has a density function, $f(x)$. 
We DO need to be able to evaluate $f(x)$ to do MCMC.
...
The Metropolis-Hastings algorithm defines a stochastic process in the parameter space. We propose a value, $y$, evaluate $f(y)$, compare to a previous value $f(x_k)$, and either accept or reject our proposed $y$ to add to a timeseries $\{x_t\}$. This timeseries is a realization of a discrete-time Markov chain. We do this for a very long time, and eventually the chain will converge to a stationary distribution coinciding with the distribution of $f(x)$. Pretty awesome.
...
The algorithm works as follows:
	...
	At iteration k:
		...
		otherwise, accept y as x_{k+1} = y with probability a = f(y)/f(x_k); 
	Repeat ad infinitum

We have most of the ingredients we need; what remains is how to propose and accept updates.
The MH algorithm is very flexible, in the sense that we can update parameters however
we like: we can sample $y \sim w(x)$, where $w(x)$ is any distribution whatsoever (or almost so).
...
current state, $x_k$, and adds a small increment in the interval $[x_k-\delta, x_k+\delta]$:
...
value to not be in the tails of $f(x)$, but I will chose a "bad" guess in the tails to illustrate the algorithm's behavior:
...
However, it is a nice feature of MCMC that in the long run it will wander to the
target distribution.
...
In Metropolis-Hastings, it is typical but not required to center the proposal
...
# Exercises:
## 0.
Modify the example above to use different values of $\delta$ in the Uniform proposal distribution.
...
## 2.
Use the acf function to determine the autocorellation in your MCMC runs. Compare this to what you should see from an equal number of samples drawn using rnorm or rpois.
	In light of this, are your MCMC samples "independent and identically distributed (iid)"?

## 3.
Modify the MH algorithm for $f(x) = p*f_1(x) + (1-p)*f_2(x)$, where $f_1$ and $f_2$ are both
	normal distributions but with means of $\mu_1=-10$ and $\mu_2=10$, respectively (you can
