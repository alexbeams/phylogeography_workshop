...
If $\mu > 0$, the process s fundamentally different. This is because sometimes the
process goes extinct before the desired number
of taxa are obtained.
Let's use the lapply function to generate a
list of simulated outputs (lapply is like a
for loop) to see how frequently this happens.
...
This suggests these trees are actually quite different. The height of the birth-death tree
is shorter than the Yule tree. With a nonzero death rate, lineages accumulate at a faster rate than
in Yule trees (conditional, of course, on attaining 50 taxa before extinction). Put another way: conditioning
on 50 taxa in a birth-death model biases us towards trees that grow rapidly. In the Yule model, lineages 
can be produced more slowly without risk of extinction.
...
Let's plot these as functions of $\lambda$ for different values of $\mu$ to see where the Maximum Likelihood
Estimates lie:

```{r}
lambdavals <- 10^seq(-3,1.1,length=30)
muvals <- 10^seq(-3,1.1,length=30)

loglik.yule.vals1 <- sapply(lambdavals, function(x) loglik.yule(c(x,0)))
loglik.yule.vals2 <- sapply(muvals, function(x) loglik.yule(c(x,0)))

par(mfrow=c(1,2))
plot(lambdavals, loglik.yule.vals1, type='l', xlab=bquote(lambda),ylab=bquote(L[Yule]))
points(x=1,y=max(loglik.yule.vals1))

plot(muvals, loglik.yule.vals2, type='l', xlab=bquote(mu),ylab=bquote(L[Yule]))
points(x=0,y=max(loglik.yule.vals2))
